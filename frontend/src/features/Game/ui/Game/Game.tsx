...
export const Game = memo(({ client, gameState, roomState }: GameProps) => {
  ...
  useWebSocketSubscription(client, `/queue/session/${roomState.id}/game-progress`, (message: Message) => {
    const data = JSON.parse(message.body);
    console.log(data)
  });

  useWebSocketSubscription(client, `/queue/session/${roomState.id}/game-results`, (message: Message) => {
    const data = JSON.parse(message.body);
    console.log(data)
  });
      ...
      <div className={classNames(cls.ç, {}, [])}>
        ...
        <div className={cls.table}>
          ...
          <div className={cls.discardBlood}>
            {gameState.bloodDiscard && <GameCard type={GameCardType.BLOOD} card={gameState.bloodDiscard} />}
          </div>
          ...
          <div className={cls.discardSand}>
            {gameState.sandDiscard && <GameCard type={GameCardType.SAND} card={gameState.sandDiscard} />}
          </div>
